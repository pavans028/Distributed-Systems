...
/**
* The PeerThreeAsServer program is a server part of a 3rd client
...
class PeerThreeAsServer implements Runnable {

	Thread tClientThree; // Thread instance for every client file download request
	...
	private static String directoryWindowsPeerThree; // Variable which stores the working directory path

	// Constructor to initialize each client and make it as a separate thread
	public PeerThreeAsServer() {
		tClientThree = new Thread(this);
		tClientThree.start();
	}
	...
	public void run() {
		...
		try {
			...
			FileReader readPeerThreeDir = new FileReader("config.properties");
			Properties properties = new Properties();
			properties.load(readPeerThreeDir);
			directoryWindowsPeerThree = properties.getProperty("directoryWindowsPeerThree"); //Peer Three Working directory
			String ipAddress = properties.getProperty("ipAddressPeerThree"); //ipaddress to host client server
		...
		while (true) {
			try {
				...
				Thread t = new Thread(new ClientAsServerCon(clientSocket, directoryWindowsPeerThree));
				t.start();
...
/**
* The ClientThree program acts a 3rd
...
public class ClientThree {
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			Properties properties = new Properties();
			properties.load(readPeerThreeDir);
			directoryWindowsPeerThree = properties.getProperty("directoryWindowsPeerThree");
	...
	*/
	public static void listFiles() {
		...
		File directory = new File(directoryWindowsPeerThree);
	...
	*/
	...
	public static void receiveFileAsAClient(String fileNameToBeReceived) {
		try {
			...
			OutputStream outputFS = new FileOutputStream((directoryWindowsPeerThree+"/"+fileNameToBeReceived));
