...
/**
* The PeerTwoAsServer program is a server part of a 2nd client
...
class PeerTwoAsServer implements Runnable {

	Thread tClientTwo; // Thread instance for every client file download request
	...
	private static String directoryWindowsPeerTwo; // Variable which stores the working directory path

	// Constructor to initialize each client and make it as a separate thread
	public PeerTwoAsServer() {
		tClientTwo = new Thread(this);
		tClientTwo.start();
	}
	...
	public void run() {
		...
		try {
			...
			FileReader readPeerTwoDir = new FileReader("config.properties");
			Properties properties = new Properties();
			properties.load(readPeerTwoDir);
			directoryWindowsPeerTwo = properties.getProperty("directoryWindowsPeerTwo");//Peer Two Working directory
		...
		while (true) {
			try {
				...
				Thread t = new Thread(new ClientAsServerCon(clientSocket, directoryWindowsPeerTwo));
				t.start();
...
/**
* The ClientTwo program acts a 2nd
...
public class ClientTwo {
	...
	public static void main(String[] args) throws IOException {
		try{
			...
			Properties properties = new Properties();
			properties.load(readPeerTwoDir);
			directoryWindowsPeerTwo = properties.getProperty("directoryWindowsPeerTwo");
	...
	*/
	 ...
	 * directoryWindowsPeerTwo ="C://test";
	 */
	...
	*/
	public static void listFiles() {
		...
		File directory = new File(directoryWindowsPeerTwo);
	...
	*/
	...
	public static void receiveFileAsAClient(String fileNameToBeReceived) {
		try {
			...
			OutputStream outputFS = new FileOutputStream((directoryWindowsPeerTwo+"/"+fileNameToBeReceived));
